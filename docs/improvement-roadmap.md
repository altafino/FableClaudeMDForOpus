...
>
> **Status 2026-07-09: Phase C + B4 harness IMPLEMENTED as v1.3** — C1 re-arm hook (`hooks/rearm.py`, SessionStart, tested on synthetic compact/resume/non-kit inputs) with C4 STATE-freshness nudge folded in; C3 installer (`scripts/install.sh`, hash-verified into a temp target); kit doctor (`scripts/kit-doctor.py`, 8-ok run on this repo); `/kit-rearm` + `/kit-doctor` skills. B4 harness: `evals/METRICS.md`, 5 tasks (all proven solvable by a correct fix), `evals/run.py` (dry-run tested end-to-end; real with/without-kit numbers require live model runs — user-initiated for cost). Remaining: run the evals, live-session hook validation, per-model overlays (A4/eval-fed), open questions.
>
...
**Design rules for all four packs:**
...
**Maintenance honesty**: these four docs are the kit's most staleness-prone content. Refresh cadence rides on F15 (version bump + upgrade note per edit), the verified-against headers, and — once Phase B lands — auditor data showing which rows actually fire.
...
1d. **Phase A4 (Addendum 3)**: author the four trap packs TRAPS-GO.md, TRAPS-ANGULAR.md, TRAPS-VUE.md, TRAPS-TAILWIND.md per the addendum's draft rows (probe-verified, verified-against headers); extend C7's split-dispatch with their manifest/file-type detection rows — one F15-versioned kit edit.
