...
| v1.5 | **Migration scripts** (sibling design doc 2026-07-10, adversarially reviewed — the reviewer caught a pre-M5 write-gate violation, fixed via prep/apply split): `scripts/migrate.sh` prep/apply + `scripts/migrate-auto.sh` + `test_migrate.sh` (green incl. zero-doc-writes gate check), MIGRATE.md script-prepped entry, README one-command migration section. v1.5.1 adds `rollback` (plan-and-confirm undo, creation-tracked, pre-existing files survive; suite 31 cases) |
